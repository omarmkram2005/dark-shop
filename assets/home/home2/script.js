...
function textApper() {
  let text = slides[vw / 100].querySelector(".textContant");
  setTimeout(() => {
    text.style.transition = "all .5s ";
    text.style.transform = "translate(-80%,-50%)";
    text.style.opacity = "1";
  }, 100);
  old = text;
}
function textDisapper() {
  setTimeout(() => {
    textApper();
  }, 1100);
  setTimeout(() => {
    old.style.transition = "none";
    old.style.transform = "translate(-80%,0%)";
    old.style.opacity = "0";
  }, 1000);
}
function goPrev() {
...
function lightBall() {
  ...
  if (vw == 0) {
    ...
    balls[2].classList.add("active");
  }
}
balls.forEach((ball) => {
  ball.addEventListener("click", function () {
    if (event.target == balls[0]) {
      vw = 0;
    } else if (event.target == balls[1]) {
      vw = 100;
    } else if (event.target == balls[2]) {
      vw = 200;
    }
    slider.style.transform = `translateX(-${vw}vw)`;
    lightBall();
