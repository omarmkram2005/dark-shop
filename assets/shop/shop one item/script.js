...
let rateCont = document.getElementById("rate");
let cat = document.getElementById("cat");
name.innerText = all_items[productId].name;
price.innerText = all_items[productId].price.toFixed(2);
discription.innerText = all_items[productId].description;
mainImage.setAttribute("src", all_items[productId].img);
all_items[productId].rate;
document.title = all_items[productId].name;
rateCont.innerHTML = "";

cat.innerText = all_items[productId].catagory;

let rate = all_items[productId].rate;
...
function addToStorage(itemId, qyt = 1) {
  if (window.localStorage.getItem("inCart")) {
    if (window.localStorage.getItem("inCart").includes(itemId)) {
      ...
      if (match) {
        ...
        let newValue = Number(currentValue) + Number(qyt);
        ...
        getSubTotal(itemId, Number(qyt));
      }
    } else {
      window.localStorage.setItem(
        "inCart",
        window.localStorage.getItem("inCart") + "," + itemId + "-" + qyt + ","
      );

      addItemFirstTime(itemId, Number(qyt));
      getSubTotal(itemId, Number(qyt));
    }
  } else {
    addButtons();
    window.localStorage.setItem("inCart", "," + itemId + "-" + qyt + ",");
...
let itemsArray = Object.keys(all_items);


function genOthers(start, end) {
  let range = Array.from({ length: end - start + 1 }, (_, i) => i + start);
  let count = Math.floor(Math.random() * 4) + 1;
  let randomNumbers = [];

  for (let i = 0; i < count; i++) {
    let randomIndex = Math.floor(Math.random() * range.length);
    randomNumbers.push(range[randomIndex]);
...
function makeItems() {
  ...
  let randNumbers = genOthers(0, Object.entries(all_items).length - 1);

  let sortedKeys = Object.keys(all_items);

  products = randNumbers.length;
  ...
  for (let id = 0; id < products; id++) {
    if (Object.keys(all_items).length > id && id < sortedKeys.length) {
      ...
      box.setAttribute("data-id", sortedKeys[randNumbers[id]]);
      box.classList.add(
        "box",
        "text-[#e3e3e3]",
        "relative",
      ...
      box.innerHTML = `
            <div class="image">
              <img class="h-full cursor-pointer w-full" src="${
                all_items[sortedKeys[randNumbers[id]]].img
              }" alt="" />
            ...
            <div class="details relative z-10 pt-[15px] pr-[22px] pb-[12px] pl-[20px] bg-[#242424]">
              <h3 class="name cursor-pointer text-[14px] mb-2 text-white rounded-sm">
               ${all_items[sortedKeys[randNumbers[id]]].name}
              </h3>
              <span class="text-sm">£${
                all_items[sortedKeys[randNumbers[id]]].price
              }.00</span>
              <span class="add off transition-all opacity-0 duration-300 text-xs absolute cursor-pointer" id="${
                sortedKeys[randNumbers[id]]
              }">
                <span class="cursor-pointer">
...
function addToCartButton() {
  ...
  addToStorage(addBut.id, addInput.value);
}
const imageContainer = document.querySelector(".image-container");
const image = imageContainer.querySelector(".hover-image");
...
imageContainer.addEventListener("mousemove", (e) => {
  ...
  const moveX = ((rect.width / 2 - offsetX) / 8).toFixed(2);
  const moveY = ((rect.height / 2 - offsetY) / 8).toFixed(2);

  image.style.transform = `scale(1.3) translate(${moveX}px, ${moveY}px)`;
});
...
function goToProLink(element) {
  ...
  const productDetailsUrl = `shop one item.html?id=${productId}`;
