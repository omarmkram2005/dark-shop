...
for (const key in all_items) {
  if (all_items[key].sale == true) {
    all_items[key].price = all_items[key].price - all_items[key].saleAmount;
  }
}
...
function addItemFirstTime(itemId, count = 1) {
  ...
  <span class="text-[#a8a8a8] text-[12px]"
  ...
  li.addEventListener("click", () => {
    ...
    const productDetailsUrl = `../../shop/shop one item/shop one item.html?id=${productId}`;
...
let sortedNames;
let sortedKeys;
function sorting(maxPrice = 100) {
  let itemsArray = Object.entries(all_items);
  let filteredItems = itemsArray.filter((item) => item[1].price <= maxPrice);
  ...
  let sortedKeys = filteredItems.map((item) => item[0]);

  let sortedPrices = filteredItems.map((item) => item[1].price);

  Prices = sortedPrices;

  let sortedNames = filteredItems.map((item) => item[1].name);
  window.localStorage.setItem("sortedKeys", sortedKeys);
  makeItems();
}

function makeItems() {
  let itemsContainer = document.getElementById("items-cont");
  let sortedKeys = window.localStorage.getItem("sortedKeys").split(",");

  let products = parseInt(window.localStorage.getItem("prodCount"));
  if (isNaN(products)) {
    products = 12;
  }
  let sortWith = document.getElementById("sort-with");
  sortWith.innerText = `${products} Products`;
  itemsContainer.innerHTML = ``;

  for (let id = 0; id < products; id++) {
    if (Object.keys(all_items).length > id && id < sortedKeys.length) {
      let box = document.createElement("div");
      box.setAttribute("data-id", sortedKeys[id]);
      ...
      box.innerHTML = `
            <div class="image">
              <img class="h-full w-full" src="${
                all_items[sortedKeys[id]].img
              }" alt="" />
              <div class="rate absolute top-1 left-2 text-[15px]"></div>
            </div>
            <div class="details relative z-10 pt-[15px] pr-[22px] pb-[12px] pl-[20px] bg-[#242424]">
              <h3 class="a text-[14px] mb-2 text-white rounded-sm">
               ${all_items[sortedKeys[id]].name}
              </h3>
              <span class="text-sm">£${
                all_items[sortedKeys[id]].price
              }.00</span>
              <span class="add off transition-all opacity-0 duration-300 text-xs absolute cursor-pointer" id="${
                sortedKeys[id]
              }">
                <span class="cursor-pointer">
                  <svg class="absolute left-[-30px] bottom-0 text-slate-100 text-[8px] pr-1 fill-slate-100" id="cart" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
            ...
            <div class="view view-off p-2 z-0 cursor-pointer absolute text-center bg-[#242424] transition-all duration-300 hover:bg-[#3897cf] w-[100%]">
              <h3>View Details</h3>
            </div>
      `;
  ...
  if (document.querySelectorAll(".page-num").length == 0) {
    getPages(sortedKeys);
  }
  boxesEvents();
  addsale();
...
function getPages(sortedKeys = window.localStorage.getItem("sortedKeys")) {
  let products = parseInt(window.localStorage.getItem("prodCount"));
  let pagesContainer = document.getElementById("pages-container");
  let pages = document.getElementById("pages");
  pagesContainer.style.display = "flex";

  let totalItems = sortedKeys.length;

  if (totalItems > products) {
    let pagesCount = Math.ceil(totalItems / products);
    let itemsInPage = [];

    for (let i = 0; i < totalItems; i += products) {
      if (!window.localStorage.getItem("pages")) {
        itemsInPage.push(Object.keys(all_items).slice(i, i + products));
      } else {
        itemsInPage.push(sortedKeys.slice(i, i + products));
      }
    }
    window.localStorage.setItem("pages", JSON.stringify(itemsInPage));

    pages.innerHTML = ``;
    for (let i = 0; i < pagesCount; i++) {
      let page = document.createElement("span");
      page.classList.add("page-num");
      let goToFirst = document.getElementById("backword");
      let goToLast = document.getElementById("forword");
      page.innerText = i + 1;
      pages.appendChild(page);

      page.addEventListener("click", function (event) {
        window.scrollTo("top", 0);
        let currentPage = parseInt(event.target.innerText) - 1;
        window.localStorage.setItem(
          "sortedKeys",
          itemsInPage[currentPage].join(",")
        );
        document
      ...
      goToFirst.addEventListener("click", function (event) {
        window.scrollTo("top", 0);
        let currentPage = 0;
      ...
      goToLast.addEventListener("click", function (event) {
        window.scrollTo("top", 0);
        let currentPage = -1;
        window.localStorage.setItem(
          "sortedKeys",
          itemsInPage[itemsInPage.length - 1].join(",")
        );
        document
          .querySelectorAll(".page-num")
          .forEach((el) => el.classList.remove("active"));
        Array.from(document.querySelectorAll(".page-num"))[
          itemsInPage.length - 1
        ].classList.add("active");
        makeItems();
      });
    }

    let activePage = parseInt(window.localStorage.getItem("curruntPage")) || 1;
    document
      .querySelectorAll(".page-num")
...
function getRate() {
  ...
  allBoxes.forEach((box) => {
    let currentBox = box;
    let id = currentBox.querySelector(".add").id;
    let rateCont = currentBox.querySelector(".rate");
    rateCont.innerHTML = "";

    let rate = all_items[id].rate;
    let fulNum = Math.floor(rate);
    let haf = rate % 1 !== 0;
    let empty = 5 - fulNum - (haf ? 1 : 0);

    for (let num = 0; num < fulNum; num++) {
      let fullStar = document.createElement("i");
      fullStar.classList.add("fas", "fa-star");
      fullStar.style.color = "#555";
      rateCont.appendChild(fullStar);
    }

    if (haf) {
      let halfStar = document.createElement("i");
      halfStar.classList.add("fas", "fa-star-half-alt");
      halfStar.style.color = "#555";
      rateCont.appendChild(halfStar);
    }

    for (let num = 0; num < empty; num++) {
      let emptyStar = document.createElement("i");
      emptyStar.classList.add("far", "fa-star");
      emptyStar.style.color = "#555";
      rateCont.appendChild(emptyStar);
    }
  });
}
...
function addsale() {
  ...
  allBoxes.forEach((box) => {
    let currentBox = box;
    let id = currentBox.querySelector(".add").id;
    if (all_items[id].sale == true) {
      let imgBox = currentBox.querySelector(".image");
...
maxSlider.setAttribute("value", Prices[Prices.length - 1]);
minSlider.setAttribute("value", 0);
rangeBar.classList.add("range-bar");
document.querySelector(".slider-container").appendChild(rangeBar);
...
function updateRangeBar() {
  ...
  maxSlider.setAttribute("min", minPrice);
  ...
  const minPercentage = ((min - minPrice) / (maxPrice - minPrice)) * 100;
  const maxPercentage = ((max - minPrice) / (maxPrice - minPrice)) * 100;

  rangeBar.style.background = `linear-gradient(
    to right,
    transparent 0%,
    transparent ${minPercentage}%,
    #454545 ${minPercentage}%,
    #454545 ${maxPercentage}%,
    transparent ${maxPercentage}%,
    transparent 100%
  )`;
}

minSlider.addEventListener("input", updateRangeBar);
maxSlider.addEventListener("input", updateRangeBar);

updateRangeBar();

let sortBy = document.getElementById("sort-by");
let sortList = document.getElementById("sort-list");
let sortedWith = document.getElementById("sortedWith");

sortBy.addEventListener("mouseover", () => {
...
let sortByShow = document.getElementById("sort-by-show");
let showList = document.getElementById("show-list");

sortByShow.addEventListener("mouseover", () => {
...
sortByShow.addEventListener("mouseleave", () => {
...
function sortByPrice(maxPrice = 100) {
  let itemsArray = Object.entries(all_items);
  ...
  let sortedKeys;
  if (window.localStorage.getItem("rev") == "true") {
    sortedKeys = itemsArray.map((item) => item[0]).reverse();
  } else {
    sortedKeys = itemsArray.map((item) => item[0]);
  }

  let sortedPrices = itemsArray.map((item) => item[1].price);

  let sortedNames = itemsArray.map((item) => item[1].name);
  sortedWith.innerText = `Price`;
  window.localStorage.setItem("sortedKeys", sortedKeys);
  window.localStorage.setItem("curMethod", "sortByPrice");

  getPages(sortedKeys);
  makeItems();
}
function sortByName() {
  const sortedKeys = Object.keys(all_items).sort((a, b) => {
    const nameA = all_items[a].name.toLowerCase();
    const nameB = all_items[b].name.toLowerCase();
  ...
  window.localStorage.setItem("sortedKeys", sortedKeys);
  window.localStorage.setItem("curMethod", "sortByName");

  getPages(sortedKeys);
  makeItems();
  sortedWith.innerText = `Name`;
}
...
function sortByPop() {
  ...
  itemsArray.sort((a, b) => {
    if (a[1].rate !== b[1].rate) {
      return a[1].rate - b[1].rate;
    }
    return a[1].name.localeCompare(b[1].name);
  });
  ...
  let sortedPrices = itemsArray.map((item) => item[1].rate);

  let sortedNames = itemsArray.map((item) => item[1].name);
  window.localStorage.setItem("sortedKeys", sortedKeys);
  window.localStorage.setItem("curMethod", "sortByPop");

  getPages(sortedKeys);
  makeItems();
  sortedWith.innerText = `Popularity`;
}
sortedWith.innerText = `${
  window.localStorage.getItem("curMethod") || "Default Order"
}`;
...
function sortByPriceRange() {
  ...
  let filteredItems = itemsArray.filter(
    (item) => item[1].price <= max && item[1].price >= min
  );
  ...
  let sortedKeys;
  if (window.localStorage.getItem("rev") == "true") {
    sortedKeys = filteredItems.map((item) => item[0]).reverse();
  } else {
    sortedKeys = filteredItems.map((item) => item[0]);
  }
  let sortedPrices = filteredItems.map((item) => item[1].price);
  ...
  window.localStorage.setItem("sortedKeys", sortedKeys);
  window.localStorage.setItem("curMethod", "sortByPriceRange");
  getPages(sortedKeys);
  makeItems();
}
let sortFilter = document.getElementById("updatePrice");
sortFilter.addEventListener("click", sortByPriceRange);

let sortPopularity = document.getElementById("sort-popularity");
sortPopularity.addEventListener("click", sortByPop);
...
let sortDef = document.getElementById("sort-defult");

sortDef.addEventListener("click", () => {
  window.location.reload();
  sortedWith.innerText = `Defult Order`;
});

let show12 = document.getElementById("show-12");
show12.addEventListener("click", () => {
  getPages();
});
let show24 = document.getElementById("show-24");
show24.addEventListener("click", () => {
  getPages();
});
let show36 = document.getElementById("show-36");

show36.addEventListener("click", () => {
  getPages();
});
let show48 = document.getElementById("show-48");
show48.addEventListener("click", () => {
  getPages();
});

showList.addEventListener("click", function (event) {
  if (event.target == show12 || event.target.parentElement == show12) {
    window.localStorage.setItem("prodCount", 12);
  } else if (event.target == show24 || event.target.parentElement == show24) {
    window.localStorage.setItem("prodCount", 24);
  } else if (event.target == show36 || event.target.parentElement == show36) {
    window.localStorage.setItem("prodCount", 36);
  } else if (event.target == show48 || event.target.parentElement == show48) {
    window.localStorage.setItem("prodCount", 48);
  }

  setProCount();
});
function setProCount() {
  let curMethod = window.localStorage.getItem("curMethod");
  if (curMethod == "sortByPrice") {
    sortByPrice();
  } else if (curMethod == "sortByName") {
    sortByName();
  } else if (curMethod == "sortByDate") {
    sortByName();
  } else if (curMethod == "sortByPop") {
    sortByPop();
  } else if (curMethod == "sortByPriceRange") {
    sortByPriceRange();
  }
}
let revItems = document.getElementById("sort-by-revurse");
let revIcon = document.getElementById("revIcon");
revItems.addEventListener("click", (e) => {
  if (e.target == revItems) {
    if (window.localStorage.getItem("rev") == "true") {
      window.localStorage.setItem("rev", "false");
      revIcon.style.transform = `rotatex(180deg)`;
    } else if (window.localStorage.getItem("rev") == "false") {
      window.localStorage.setItem("rev", "true");
      revIcon.style.transform = `rotatex(0deg)`;
    }
    setProCount();
  }
});
...
function goToProLink(element) {
  ...
  const productDetailsUrl = `../shop one item/shop one item.html?id=${productId}`;
